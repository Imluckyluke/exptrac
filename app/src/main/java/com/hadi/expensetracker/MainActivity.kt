...
class MainActivity : AppCompatActivity() {
    ...
    private fun showManageCategoriesDialog(chipGroup: ChipGroup) {
        ...
        fun refreshCustomList() {
            ...
            val customs = dbHelper.getCustomCategories()
            dialogBinding.tvNoCustomCategories.visibility = if (customs.isEmpty()) View.VISIBLE else View.GONE
            for (cat in customs) {
                val rowBinding = ItemCustomCategoryBinding.inflate(layoutInflater, dialogBinding.llCustomCategories, false)
                val categoryId = "custom_${cat.id}"
                rowBinding.tvCategoryLabel.text = cat.label
                rowBinding.catDot.background.mutate().setTint(
                    ContextCompat.getColor(this, Category.colorResOf(categoryId))
                )
                rowBinding.btnDeleteCategory.setOnClickListener {
                    val fallback = selectedCategory(chipGroup)
                    dbHelper.deleteCustomCategory(cat.id)
                    Category.refresh(this, dbHelper)
                    refreshCustomList()
                    populateCategoryChips(chipGroup, if (fallback == categoryId) Category.DEFAULT else fallback)
        ...
        dialogBinding.btnAddCategory.setOnClickListener {
            ...
            val newId = Category.add(this, dbHelper, label)
    ...
    /** Cross-fades the list/empty-state area instead of leaving every row to pop in and out
     * individually, which looked chaotic since a whole day's worth of items changes at once. */
